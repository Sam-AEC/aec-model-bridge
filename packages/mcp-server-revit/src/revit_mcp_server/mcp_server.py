...
def build_registry() -> tuple[ProviderRegistry, ApprovalProvider, JobManager, ModuleRegistry, WorkspaceMonitor]:
    ...
    Factored out so the stdio MCP server (below) and the panel's local HTTP shim
    (panel_server.py) build an identical registry without duplicating this ~60
    lines of provider wiring. Each caller gets its own independent instance —
    that's fine, since ApprovalGate persists plans to disk under the shared
    workspace directory, so plans created via one process are still visible to
    the other.
    """
