    ...
    public static class ReflectionHelper
    {
        ...
        public static object ParseArgument(JsonElement arg, Document doc)
        {
            switch (arg.ValueKind)
            {
                ...
                case JsonValueKind.Object:
                    ...
                    {
                        return new Point3D(arg.GetProperty("x").GetDouble(), arg.GetProperty("y").GetDouble(), arg.GetProperty("z").GetDouble());
                    }
                    return null;
                default:
                    return null;
        ...
        public static Type ResolveType(string typeName)
        {
            ...
            var assemblies = AppDomain.CurrentDomain.GetAssemblies()
                .Where(a => a.FullName.StartsWith("Autodesk.Navisworks") || a.FullName.StartsWith("System"));
            
            foreach (var asm in assemblies)
            {
                var type = asm.GetType(typeName) ?? asm.GetType("Autodesk.Navisworks.Api." + typeName) ?? asm.GetType("Autodesk.Navisworks.Api.Interop." + typeName);
