...
SCRIPT_PATH = os.path.join(os.path.dirname(__file__), "omega_tower.py")

def main():
    if not os.path.exists(SCRIPT_PATH):
        sys.exit("Cannot find omega_tower.py: " + SCRIPT_PATH)
    ...
    print("Sending omega_tower.py script to Rhino ({0} lines)...".format(len(code.splitlines())))
    try:
        r = requests.post(
            EXECUTE_URL,
        ...
        sys.exit("Timed out running omega_tower.py script inside Rhino.")
    except Exception as e:
